...
class StockAnalyzer:
    ...
    def tavily_news(self,k=4):
        ...
        news_link = list(map(lambda i, x: f"News {i+1}: {x['url']}", range(len(response['results'])), response['results']))[:k]
...
def analyze_stock(symbol: str, openai_key: str):
    ...
    results = {
        ...
        'News_link' : analyzer.tavily_news(),
...
if __name__ == "__main__":
    ...
    # Run the application
    uvicorn.run("app_html_main:app", host="0.0.0.0", port=8000, reload=True)
